(function () {
  ...
  // Upload original file to Cloudinary (no transformations)
  ...
  function init(root) {
    ...
    const enforcement = (root.dataset.enforcement || 'soft').toLowerCase(); // soft | strict
    const previewMode = (root.dataset.preview || 'on').toLowerCase(); // on | off
    ...
    const qualityMode = (root.dataset.qualityMode || 'warn').toLowerCase(); // warn | never | block
    ...
    async function handleFile(idx, file) {
      ...
      if (!ALLOWED.includes(file.type)) {
        setStatus(root, idx, 'Please upload PNG, JPG, SVG.', 'error');
        return;
      }

      const mb = file.size / (1024 * 1024);
      if (mb > MAX_MB) {
        setStatus(root, idx, `File too large (${mb.toFixed(1)}MB). Max ${MAX_MB}MB.`, 'error');
        return;
      }
      ...
      img.onload = async () => {
        ...
        if (previewMode === 'on' && previewWrap && previewImg) {
          ...
          if (meta) meta.textContent = `${file.name} • ${w}×${h}px`;
        } else {
          setStatus(root, idx, `${file.name} selected (${w}×${h}px). Uploading…`, 'ok');
        }

// --- QUALITY CHECK (uses LONG SIDE, skips SVG) ---
...
if (qualityMode !== 'never' && !isSvg && longSide > 0 && longSide < minWidth) {
  lowResWarn = true;
  lowResMsg = `Image is ${w}×${h}px. Recommended: ${minWidth}px+ on the longest side for best print.`;
  ...
  setStatus(root, idx, 'Uploading…', 'ok');
}
        if (!cloud || !preset) {
  ...
  setStatus(root, idx, 'Upload complete.', 'ok');
          ...
          if (meta && previewMode === 'on') {
            ...
            meta.textContent = `${file.name} • ${w}×${h}px • Uploaded${lowResWarn ? ' • Low resolution' : ''}`;
          }
        } catch (err) {
          console.error(err);
          setStatus(root, idx, 'Upload failed. Please try again.', 'error');
          if (urlEl) urlEl.value = '';
          if (pidEl) pidEl.value = '';
        }
      };
